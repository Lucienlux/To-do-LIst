...
const Todolist = () => {
  ...
  function editTask(index: number, taskList: string[], taskSetter: Dispatch<SetStateAction<string[]>>) {
          ...
          <div className="flex">
            ...
            <button
              ...
              className={`font-mono text-white rounded-md p-2 ml-2 ${
                submittedTasks.trim() ? 'bg-purple-900' : 'bg-gray-400 cursor-not-allowed'
              }`}
            >
              Add
            </button>
        ...
        <div key={crypto.randomUUID()} className="flex justify-start items-center mt-4 mx-10 grid-rows-1">
          <ol className="flex flex-col items-start gap-4">
            {tasks.map((value, index) => (
              <div
                onDoubleClick={() => editTask(index, tasks, setTasks)}
            ...
            {completedTasks.map((value, index) => (
              ...
              >
                <div className="flex items-center gap-2 border p-2 rounded">
                  <li className="font-mono">{`${index + 1}. ${value}`}</li>
                  <button
                    onClick={() => deleteTask(index, completedTasks, setCompletedTasks)}
                    className="border rounded p-0.5"
                  >
                    X
                  </button>
